...
export function countLanguages(list: Developer[]) {
    return list.reduce<LanguageCounts>((languageCounts, { language }) => {
        if (!language) {
            return languageCounts;
        }

        const developerLanguage = languageCounts[language];

        if (developerLanguage) {
            return { ...languageCounts, [language]: developerLanguage + 1 };
        }

        return { ...languageCounts, [language]: 1 };
    }, {});
}
